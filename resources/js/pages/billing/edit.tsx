...
export default function BillingEdit({ appointment, chargeMasters }: { appointment: Appointment; chargeMasters: ChargeMaster[] }) {
    ...
    const { data, setData, put, processing, errors } = useForm({
        ...
        amount_1: Number(appointment.amount_1 ?? 0) > 0 ? String(appointment.amount_1) : getAmountForService(appointment.appointment_reason),

        amount_2: Number(appointment.amount_2 ?? 0) > 0 ? String(appointment.amount_2) : getAmountForService(appointment.service_2),

        amount_3: Number(appointment.amount_3 ?? 0) > 0 ? String(appointment.amount_3) : getAmountForService(appointment.service_3),
    ...
    const totalAmount = Number(data.amount_1 || 0) + Number(data.amount_2 || 0) + Number(data.amount_3 || 0);
    ...
    const labelClass = 'mb-2 block text-sm font-medium text-slate-800 dark:text-foreground';
    ...
    return (
        ...
        >
            ...
            <div className="space-y-6 bg-[#F8FAFC] p-6 dark:bg-background">
                {/* Page heading */}
                <div>
                    <h1 className="text-2xl font-semibold text-slate-900 dark:text-foreground">Edit Billing Visit</h1>

                    <p className="text-sm text-slate-500 dark:text-muted-foreground">Update selected services and billing amounts.</p>
                </div>
                ...
                >
                    ...
                    <div className="overflow-x-auto rounded-xl border border-blue-100 dark:border-border">
                        <table className="w-full border-collapse text-sm">
                                ...
                                <tr className="transition hover:bg-blue-50/50 dark:hover:bg-accent">
                                    <td className="border border-blue-100 px-4 py-3 dark:border-border">
                                        <select
                                            ...
                                            onChange={(e) => handleServiceChange('appointment_reason', 'amount_1', e.target.value)}
                                        ...
                                        {errors.appointment_reason && <p className={errorClass}>{errors.appointment_reason}</p>}
                                    </td>
                                ...
                                <tr className="transition hover:bg-blue-50/50 dark:hover:bg-accent">
                                    <td className="border border-blue-100 px-4 py-3 dark:border-border">
                                        <select
                                            ...
                                            onChange={(e) => handleServiceChange('service_2', 'amount_2', e.target.value)}
                                            className={selectClass}
                                        >
                                ...
                                <tr className="transition hover:bg-blue-50/50 dark:hover:bg-accent">
                                    <td className="border border-blue-100 px-4 py-3 dark:border-border">
                                        <select
                                            ...
                                            onChange={(e) => handleServiceChange('service_3', 'amount_3', e.target.value)}
                                            className={selectClass}
                                        >
                                            <option value="">Select additional service...</option>
                            ...
                            <tfoot className="bg-[#EAF5FF] font-semibold text-slate-900 dark:bg-muted dark:text-foreground">
                                <tr>
                                    <td className="border border-blue-100 px-4 py-3 dark:border-border">Total</td>

                                    <td className="border border-blue-100 px-4 py-3 dark:border-border">Rs {totalAmount.toFixed(2)}</td>
                                </tr>
                            </tfoot>
                        </table>
                    </div>
                    ...
                    <div className="grid gap-6 md:max-w-xl">
                        <div>
                            ...
                            <select value={data.responsibility} onChange={(e) => setData('responsibility', e.target.value)} className={selectClass}>
                        ...
                        <div>
                            ...
                            <select value={data.claim_status} onChange={(e) => setData('claim_status', e.target.value)} className={selectClass}>
