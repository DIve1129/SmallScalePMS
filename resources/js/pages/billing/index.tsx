...
export default function BillingIndex({ appointments = [], from, to }: { appointments?: Appointment[]; from?: string; to?: string }) {
    ...
    return (
        ...
        >
            ...
            <div className="space-y-6 bg-[#F8FAFC] p-6 text-slate-800 dark:bg-background dark:text-foreground">
                {/* Page heading */}
                <div>
                    <h1 className="text-2xl font-semibold text-slate-900 dark:text-foreground">Billing Module</h1>

                    <p className="text-sm text-slate-500 dark:text-muted-foreground">Billing queue for completed and no-show appointments</p>
                </div>

                {/* Date filters */}
                <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:gap-4">
                    <div className="flex flex-col gap-2">
                        <label htmlFor="billing-from-date" className="text-sm text-slate-500 dark:text-muted-foreground">
                            From
                        </label>
                        ...
                        <input
                            ...
                            className="w-full rounded-lg border border-blue-100 bg-white px-4 py-3 text-sm text-slate-800 transition outline-none focus:border-[#2563EB] focus:ring-2 focus:ring-blue-100 dark:border-border dark:bg-background dark:text-foreground"
                        />
                    </div>

                    <div className="flex flex-col gap-2">
                        <label htmlFor="billing-to-date" className="text-sm text-slate-500 dark:text-muted-foreground">
                            To
                        </label>
                    ...
                    <div className="divide-y divide-blue-100 dark:divide-border">
                        {appointments.length > 0 ? (
                            appointments.map((appointment) => (
                                ...
                                >
                                    <div className="col-span-1">{appointment.appointment_id}</div>

                                    <div className="col-span-1">{appointment.patient_id ?? '-'}</div>

                                    <div className="col-span-2">{appointment.patient_name ?? '-'}</div>

                                    <div className="col-span-1">{appointment.doctor_name ?? '-'}</div>

                                    <div className="col-span-2">{appointment.appointment_date ?? '-'}</div>

                                    <div className="col-span-1">{appointment.appointment_reason ?? '-'}</div>

                                    <div className="col-span-1">{formatCurrency(appointment.total_amount)}</div>

                                    <div className="col-span-1 font-medium">{formatCurrency(appointment.balance)}</div>

                                    <div className="col-span-1">{appointment.responsibility ?? 'Patient'}</div>
                            ...
                            <div className="px-5 py-10 text-sm text-slate-500 dark:text-muted-foreground">No billable appointments found.</div>
                        )}
                    </div>
                </div>
            </div>
