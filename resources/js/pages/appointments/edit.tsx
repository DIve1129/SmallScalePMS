...
export default function EditAppointment({ appointment, patients = [] }: { appointment: Appointment; patients?: Patient[] }) {
        ...
        >
            ...
            <div className="min-h-full bg-[#F8FAFC] p-6 text-slate-800 dark:bg-background dark:text-foreground">
                <div className="mx-auto w-full max-w-3xl space-y-6">
                    ...
                    <div>
                        <h1 className="text-2xl font-semibold text-slate-900 dark:text-foreground">Edit Appointment</h1>

                        <p className="mt-1 text-sm text-slate-500 dark:text-muted-foreground">Update appointment details and status.</p>
                    </div>
                    ...
                    >
                        <div>
                            ...
                            <select value={data.patient_id} onChange={(event) => setData('patient_id', event.target.value)} className={selectClass}>
                        ...
                        <div>
                            ...
                            <input
                                ...
                                onChange={(event) => setData('doctor_id', event.target.value)}
                                className={inputClass}
                            />

                            {errors.doctor_id && <p className="mt-2 text-sm text-red-500">{errors.doctor_id}</p>}
                        </div>
                        ...
                        <div>
                            ...
                            <input
                                ...
                                onChange={(event) => setData('app_reason', event.target.value)}
                                className={inputClass}
                            />

                            {errors.app_reason && <p className="mt-2 text-sm text-red-500">{errors.app_reason}</p>}
                        </div>

                        <div>
                            <label className={labelClass}>Status</label>

                            <select value={data.status} onChange={(event) => setData('status', event.target.value)} className={selectClass}>
                        ...
                        <div>
                            <label className={labelClass}>{data.status === 'Rescheduled' ? 'New Date & Time' : 'Date & Time'}</label>
                            ...
                            {errors.scheduled_at && <p className="mt-2 text-sm text-red-500">{errors.scheduled_at}</p>}
                        </div>
