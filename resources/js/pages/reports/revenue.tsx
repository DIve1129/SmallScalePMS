...
export default function RevenueReport({ rows, filters, totalRevenue }: RevenueReportProps) {
        ...
        >
            ...
            <div className="min-h-full space-y-6 bg-[#F8FAFC] p-6 text-slate-800 dark:bg-background dark:text-foreground">
                ...
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <h1 className="text-2xl font-semibold text-slate-900 dark:text-foreground">Revenue Collection Report</h1>

                        <p className="mt-1 text-sm text-slate-500 dark:text-muted-foreground">
                            Payments collected for services provided within the selected date-of-service range.
                        </p>
                    </div>
                ...
                <div className="rounded-xl border border-blue-100 bg-white p-5 shadow-sm dark:border-border dark:bg-card">
                    <h2 className="mb-4 text-sm font-semibold tracking-wide text-slate-500 uppercase dark:text-muted-foreground">Report Filters</h2>
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                    <div className="rounded-xl border border-blue-100 bg-white p-5 shadow-sm dark:border-border dark:bg-card">
                        ...
                        <p className="mt-2 text-3xl font-semibold text-[#2563EB] dark:text-foreground">{formatCurrency(totalRevenue)}</p>
                    </div>

                    <div className="rounded-xl border border-blue-100 bg-white p-5 shadow-sm dark:border-border dark:bg-card">
                        <p className="text-xs font-medium tracking-wider text-slate-500 uppercase dark:text-muted-foreground">Paid Service Entries</p>

                        <p className="mt-2 text-3xl font-semibold text-slate-900 dark:text-foreground">{rows.length}</p>
                    </div>

                    <div className="rounded-xl border border-blue-100 bg-white p-5 shadow-sm dark:border-border dark:bg-card">
                        <p className="text-xs font-medium tracking-wider text-slate-500 uppercase dark:text-muted-foreground">DOS Range</p>
                ...
                <div className="overflow-hidden rounded-xl border border-blue-100 bg-white shadow-sm dark:border-border dark:bg-card">
                    <div className="border-b border-blue-100 p-5 dark:border-border">
                        <h2 className="font-semibold text-slate-900 dark:text-foreground">Revenue Collection Details</h2>

                        <p className="mt-1 text-sm text-slate-500 dark:text-muted-foreground">
                            Each service and its corresponding payment are displayed as a separate row.
                        </p>
                    </div>
                                ...
                                {rows.length > 0 ? (
                                    rows.map((row) => (
                                        ...
                                        >
                                            ...
                                            <td className="px-5 py-4 text-right font-medium">{formatCurrency(row.payment)}</td>
                                        </tr>
                                    ))
                                ) : (
                                    <tr>
                                        <td colSpan={5} className="px-5 py-12 text-center text-slate-500 dark:text-muted-foreground">
                                            No collected revenue was found for the selected DOS range.
                                        </td>
                                    </tr>
                                )}
                            </tbody>
